package org.geoframe.geoet.inout;
...
import org.geoframe.geoet.transpiration.methods.PressureMethods;
...
public class InputReaderMain {
	...
	@Description("Root Depth can be evaluated in different way" + " costantValue - rootGrowth")
	@In
	...
	public void process() throws Exception {
		...
		for (Entry<Integer, double[]> entry : entrySet) {
			...
			if (inLeafAreaIndex != null) {
				input.leafAreaIndex = inLeafAreaIndex.get(ID)[0];
			}
			if (input.leafAreaIndex == nullValue) {
				input.leafAreaIndex = parameters.defaultLeafAreaIndex;
			}
			...
			if (step == 0) {
				...
				if (input.rootDepth == nullValue) {
					variables.rootDepth = parameters.defaultRootDepth;
				}
			}

			if ("rootGrowth".equals(rootType)) {
				if (inRootDepth != null) {
					variables.rootDepth = inRootDepth.get(ID)[0];
				}
				if (variables.rootDepth == nullValue) {
					variables.rootDepth = parameters.defaultRootDepth;
				}
			}
			...
			if (step == 0) {
				...
				if (input.canopyHeight == nullValue) {
					variables.canopyHeight = parameters.defaultCanopyHeigth;
				}
			}

			if ("canopyHeightGrowth".equals(canopyHeightType)) {
				if (inCanopyHeight != null) {
					variables.canopyHeight = inCanopyHeight.get(ID)[0];
				}
				if (variables.canopyHeight == nullValue) {
					variables.canopyHeight = parameters.defaultCanopyHeigth;
				}
			}
			...
			if (input.shortWaveRadiationDirect == nullValue) {
				input.shortWaveRadiationDirect = parameters.defaultShortWaveRadiationDirect;
			}

			if (inShortWaveRadiationDiffuse != null) {
				input.shortWaveRadiationDiffuse = inShortWaveRadiationDiffuse.get(ID)[0];
			}
			if (input.shortWaveRadiationDiffuse == nullValue) {
				input.shortWaveRadiationDiffuse = 0.159 * input.shortWaveRadiationDirect;
			}

			if (inLongWaveRadiation != null) {
				input.longWaveRadiation = inLongWaveRadiation.get(ID)[0];
			}
			if (input.longWaveRadiation == nullValue) {
				input.longWaveRadiation = leafparameters.longWaveEmittance * parameters.stefanBoltzmannConstant
						* pow(input.airTemperature, 4);
			}

			if (inNetRadiation != null) {
				input.netRadiation = inNetRadiation.get(ID)[0];
			}
			if (input.netRadiation == nullValue) {
				input.netRadiation = defaultNetRadiation;
			}

			input.windVelocity = parameters.defaultWindVelocity;
			if (inWindVelocity != null) {
				input.windVelocity = inWindVelocity.get(ID)[0];
			}
			if (input.windVelocity == nullValue) {
				input.windVelocity = parameters.defaultWindVelocity;
			}
			if (input.windVelocity == 0) {
				input.windVelocity = parameters.defaultWindVelocity;
			}
			...
			if (inRelativeHumidity != null) {
				input.relativeHumidity = inRelativeHumidity.get(ID)[0];
			}
			if (input.relativeHumidity == nullValue) {
				input.relativeHumidity = parameters.defaultRelativeHumidity;
			}

			input.soilFlux = parameters.defaultSoilFlux;
			if (inSoilFlux != null) {
				input.soilFlux = inSoilFlux.get(ID)[0];
			}
			if (input.soilFlux == nullValue) {
				input.soilFlux = parameters.defaultSoilFlux;
			}

			input.soilMoisture = parameters.defaultSoilMoisture;
			if (inSoilMoisture != null) {
				input.soilMoisture = inSoilMoisture.get(ID)[0];
			}
			if (input.soilMoisture == nullValue) {
				input.soilMoisture = parameters.defaultSoilMoisture;
			}
			...
			if (inAssimilationRate != null) {
				input.assimilationRate = inAssimilationRate.get(ID)[0];
			}
			if (input.assimilationRate == nullValue) {
				input.assimilationRate = parameters.defaultAssimilationRate;
			}
		...
		try {
			while (iterator.hasNext()) {
				...
				if (centroidElevation != null) {
					try {
						z = ((Number) feature.getAttribute(centroidElevation)).doubleValue();
					} catch (NullPointerException e) {
						throw new Exception("kriging.noPointZ");
					}
				}
				coordinate.z = z;
