...
public class RadiationMethod {
	...
	// Queste due righe le ho aggiunte nperchè stavo iniziando a fare la modifica di
	// restyle del modello
	...
	public double computeLongWaveRadiationBalance(double leafSide, double longWaveEmittance, double airTemperature,
			double leafTemperature, double stefanBoltzmannConstant) {
		// Compute the net long wave radiation i.e. the incoming minus outgoing [J m-2
		// s-1]
		longWaveRadiation = 4 * leafSide * longWaveEmittance * stefanBoltzmannConstant
	...
	public double computeAbsorbedRadiationSunlit(double leafAreaIndex, double solarElevationAngle,
		...
		canopyReflectionCoefficientBeam = 1
				- exp((-2 * 0.041 * directExtinctionCoefficientInCanopy) / (1 + directExtinctionCoefficientInCanopy)); // ρcbP

		// Ryu et all 2011 eq.3
		directAbsorbedRadiation = shortWaveRadiationDirect * (1 - parameters.leafScatteringCoefficient)
				* (1 - exp(-directExtinctionCoefficientInCanopy * leafAreaIndex));

		// Ryu et all 2011 eq.4
		diffuseAbsorbedRadiation = shortWaveRadiationDiffuse * (1 - parameters.canopyReflectionCoefficientDiffuse) * (1
				- exp(-(parameters.diffuseExtinctionCoefficient + directExtinctionCoefficientInCanopy) * leafAreaIndex))
	...
	public double computeAbsorbedRadiationShadow(double leafAreaIndex, double solarElevationAngle,
			double shortWaveRadiationDirect, double shortWaveRadiationDiffuse) {
		parameters = Parameters.getInstance();
		...
		diffuseAbsorbedRadiationShadow = shortWaveRadiationDiffuse * (1 - canopyReflectionCoefficientBeam) * (1
		...
		scatteredAbsorbedRadiationShadow = shortWaveRadiationDirect * ((1 - canopyReflectionCoefficientBeam) * (1
	...
	public double computeSunlitLeafAreaIndex(String typeOfCanopy, double leafAreaIndex, double solarElevationAngle) {
		...
		if ("grassland".equals(typeOfCanopy)) {
			...
			directExtinctionCoefficientInCanopy = 0.5 / solarElevationAngle;
			sunlitLeafAreaIndex = (1 - exp(-directExtinctionCoefficientInCanopy * leafAreaIndex))
